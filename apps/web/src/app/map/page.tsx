...
// Custom icon for walk and wheels
const WalkIcon = (color: string) => {
  if (typeof window !== "undefined" && window.L) {
    return window.L.divIcon({
      className: "custom-div-icon",
...
const routes: Record<string, Route & { friends?: string[] }> = {
  shortest: {
    name: "Weekend walk",
...
export default function Home() {
  ...
  const selectedPathKey = searchParams.get("path") || "shortest";
  const selectedRouteData = routes[selectedPathKey];

  // Calculate bounds for the selected route
  let mapBounds: [[number, number], [number, number]] | undefined = undefined;
  let routeBusinesses: Business[] = [];
  if (selectedRouteData) {
    const businessSet = selectedRouteData.mode === 'walk' ? walkingBusinesses : wheelsBusinesses;
    routeBusinesses = selectedRouteData.businesses.map(i => businessSet[i]);
  ...
  // Add color to routeBusinesses for selected route
  const routeBusinessesWithColor = useMemo(() => {
    return routeBusinesses.map((biz, i) => ({
      ...biz,
      color: markerColors[i % markerColors.length]
    }));
  }, [routeBusinesses]);
  ...
  const routeBusinessesForMap = selectedRouteData ? routeBusinessesWithColor : [];
  ...
  useEffect(() => {
    if (selectedRouteData && adventureStarted && routeBusinessesForMap.length > 0) {
      const allVisited = routeBusinessesForMap.every(biz => visitedLocations.has(biz.name));
      ...
      if (allVisited && !showCompletionModal) {
        ...
        const estimatedDistance = routeBusinessesForMap.length * 0.5; // Rough estimate: 0.5 miles per location
  ...
  }, [selectedRouteData, adventureStarted, visitedLocations, routeBusinessesForMap, showCompletionModal, adventureStartTime, totalPoints]);
      ...
      <main className="min-h-screen bg-[#f5f7fa] flex flex-col">
        ...
        <div className="flex h-[calc(100vh-80px)]">
          ...
          <div className="flex-1 relative pr-80">
              ...
              <div className="absolute top-6 left-6 z-[1000]">
                <div className="bg-white rounded-xl shadow-lg px-4 py-3">
                  ...
                  <div className="text-xs text-gray-600">
                    {selectedRouteData.totalPoints} total points
                  </div>
                </div>
              </div>
            )} */}
            ...
            {(selectedRouteData ? routeBusinessesForMap : businessWithPoints).map((biz, index) => {
              let icon;
              if (selectedRouteData) {
                icon = selectedRouteData.mode === 'walk' ? WalkIcon(biz.color) : WheelsIcon(biz.color);
              } else {
                icon = MiniDotIcon(biz.color);
              }
                        ...
                        {businessInfo?.reviews && businessInfo.reviews.length > 0 && (
                          <div className="bg-gray-50 p-2 rounded text-sm">
                            <div className="flex items-center mb-1">
                              <span className="font-semibold text-gray-600">{businessInfo.reviews[0].user}</span>
                              <div className="flex">
                                {[...Array(5)].map((_, i) => (
            ...
            {selectedRouteData && routeBusinessesForMap.length > 0 && (
              <Polyline
                positions={routeBusinessesForMap.map(biz => [biz.lat, biz.lng])}
                color="#4a90e2"
                weight={4}
                opacity={0.8}
              />
        ...
        {/* Sidebar */}
        <div className="w-80 bg-white shadow-lg overflow-y-auto h-full fixed right-0 top-19">
          <div className="p-6 pb-20">
            <h2 className="text-2xl font-bold text-[#4a90e2] mb-4">{selectedRouteData.name}</h2>
            <h3 className="text-lg font-semibold mb-3 ml-2">
            ...
            {selectedRouteData && (
              <div className="mb-6 ml-2 text-sm">
                {(() => {
                  // Calculate distance and time for the selected route
                  let totalDistance = 0;
                  for (let i = 0; i < routeBusinessesForMap.length - 1; i++) {
                    const lat1 = routeBusinessesForMap[i].lat;
                    const lng1 = routeBusinessesForMap[i].lng;
                    const lat2 = routeBusinessesForMap[i + 1].lat;
                    const lng2 = routeBusinessesForMap[i + 1].lng;
                    const R = 3959;
                  ...
                  return (
                    <>
                      ...
                      <div className="text-lg font-bold text-blue-600 mt-2">
                        Estimated Time: ~{estimatedTimeMinutes} min
                      </div>
                    </>
                  );
                })()}
            ...
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-[#4a90e2] mb-3">
                {selectedRouteData ? "Your Path Locations" : "All Nearby Locations"}
              </h3>
              <div className="space-y-2">
                {(selectedRouteData ? routeBusinessesForMap : businessWithPoints).map((biz, index) => {
                  const isVisited = visitedLocations.has(biz.name);
                  return (
                    <div key={biz.name} className={`flex items-center p-3 rounded-lg transition bg-[#f5f7fa] border ${
            ...
            {/* Path Buddies Section */}
            {selectedRouteData && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-[#4a90e2] mb-3">Your Path Buddies</h3>
                <div className="space-y-2">
                  {(selectedRouteData.friends && selectedRouteData.friends.length > 0) ? (
                    selectedRouteData.friends.map((buddy, idx) => (
                      <div key={buddy} className="flex items-center p-3 rounded-lg transition bg-[#f5f7fa] border">
                        <div 
                          className="w-3 h-3 rounded-full mr-3"
                          style={{ backgroundColor: markerColors[idx % markerColors.length] }}
                        ></div>
                        <div className="flex-1">
                          <div className="font-semibold text-[#3a4a5d] flex items-center">{buddy}</div>
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="text-gray-500 text-sm">No buddies on this path!</div>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              {adventureStarted && selectedRouteData && (
                <button 
  ...
  if (showWalkStatsModal) {
    ...
    const progressPercentage = routeBusinessesForMap.length > 0 ? (visitedLocations.size / routeBusinessesForMap.length) * 100 : 0;

    return (
      <div className="fixed inset-0 z-[2300] flex flex-col items-center justify-center bg-[#f5f7fa]">
        {/* Blurred background content */}
        <div className="absolute inset-0 backdrop-blur-sm opacity-30 pointer-events-none">
          <div className="flex h-[calc(100vh-80px)]">
        ...
        <div className="relative z-[2400] bg-white rounded-xl shadow-lg p-8 w-full max-w-md max-h-[90vh] overflow-y-auto flex flex-col items-center">
          ...
          {selectedRouteData && (
            <div className="w-full bg-gray-50 rounded-lg p-4 mb-6">
              ...
              <div className="text-sm text-gray-600">
                {visitedLocations.size} of {routeBusinessesForMap.length} locations completed
              </div>
            </div>
          )}
              ...
              <section className="flex-1 flex flex-col gap-6">
                ...
                {visitedLocations.size > 0 && (
                  <div className="w-full bg-[#f5f7fa] rounded p-4">
                    ...
                    <div className="space-y-2">
                      {routeBusinessesForMap.filter(biz => visitedLocations.has(biz.name)).map((biz, index) => (
                        <div key={biz.name} className="flex items-center justify-between p-2 bg-white rounded border">
                          <div className="flex items-center">
                            <div 
                              className="w-3 h-3 rounded-full mr-3"
                ...
                {/* Remaining Locations */}
                {selectedRouteData && routeBusinessesForMap.filter(biz => !visitedLocations.has(biz.name)).length > 0 && (
                  <div className="w-full bg-[#f5f7fa] rounded p-4">
